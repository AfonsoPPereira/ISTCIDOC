(function($) {
    ...
    var setSelectorsInFields = function (location_item) {
        if (!location_item['literal']){
            $('.add-value.button.o-icon-literal').eq(0).remove();
       }
        if (!location_item['resource']){
            $('.add-value.button.o-icon-resource').eq(0).remove();
       }
        if (!location_item['uri']){
            $('.add-value.button.o-icon-uri').eq(0).remove();
       }
       if (!location_item['location']){
            $('.add-value.button.o-icon-location').eq(0).remove();
        }
    };
    ...
    var initPage = function() {
        makeNewField(location_item['property_term'], location_item['property_label'], 
            location_item['property_description']).find('.values')
        .append(makeDefaultValue(location_item['property_term'], location_item['default_term']));      

        $.when(applyResourceTemplate(false)).done(function () {
            $('#properties').closest('form').trigger('o:form-loaded');
        });

        setSelectorsInFields(location_item);
    };
})(jQuery);
