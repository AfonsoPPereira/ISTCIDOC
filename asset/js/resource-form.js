(function($) {
    ...
    $(document).ready( function() {
        ...
        $('#properties').on('click', '.add-value', function(e) {
            ...
            var field2 = typeButton.parent().prev().find("div:first");

            if (field2.data('data-type') != $(this).attr('data-type')){
                field.find('.values').empty().append(value);
            }
    ...
    $(document).on('o:prepare-value', function(e, type, value, valueObj, namePrefix) {
        ...
        value.find(':input').each(function () {
            ...
            $(this).attr('name', 'ist_cidoc_fieldset[' + namePrefix + '][value][]')
     ...
     */
    var makeNewField = function(property, label, description) {
        ...
        field.find('.field-term').text(property);
        field.find('.field-description').prepend(description);
        field.data('property-term', property);
        // Adding the attr because selectors need them to find the correct field
        // and count when adding more.
        field.attr('data-property-term', property);
    ...
    var setSelectorsInFields = function (properties_array) {
        for (var i = properties_array.length - 1; i >= 0; --i){
           if (!properties_array[i]['literal']){
                $('.add-value.button.o-icon-literal').eq(i).remove();
           }
            if (!properties_array[i]['resource']){
                $('.add-value.button.o-icon-resource').eq(i).remove();
           }
            if (!properties_array[i]['uri']){
                $('.add-value.button.o-icon-uri').eq(i).remove();
           }
           if (!properties_array[i]['location']){
                $('.add-value.button.o-icon-location').eq(i).remove();
           }
        }
    };
    ...
    var initPage = function() {
        for (var i = 0; i < properties_array.length; i++){
            makeNewField(properties_array[i]['property_term'], properties_array[i]['property_label'], 
                properties_array[i]['property_description']).find('.values')
            .append(makeDefaultValue(properties_array[i]['property_term'], properties_array[i]['default_term']));
            //alert(i);
        }        
        ...
        setSelectorsInFields(properties_array);
    };
})(jQuery);
